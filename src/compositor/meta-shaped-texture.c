...
meta_shaped_texture_init (MetaShapedTexture *self)
{
  ...
  priv->transform = META_SHAPED_TEXTURE_TRANSFORM_NORMAL;
}
...
static int
get_dest_width (MetaShapedTexture *self)
{
  ...
  return priv->dest_width;
}

static int
get_dest_height (MetaShapedTexture *self)
{
  MetaShapedTexturePrivate *priv = self->priv;
  ...
  if (unobscured_region)
    {
      ...
      if (priv->texture)
        {
          width = get_dest_width (self);
          height = get_dest_height (self);
        }
      else
        {
                   ...
                   CoglContext       *ctx)
{
  ...
  if (priv->transform != META_SHAPED_TEXTURE_TRANSFORM_NORMAL)
    {
      CoglMatrix matrix;
      CoglEuler euler;
...
meta_shaped_texture_paint (ClutterActor *actor)
{
  ...
  clutter_actor_get_scale (actor, &tex_scale, NULL);
  dest_width = get_dest_width (stex);
  dest_height = get_dest_height (stex);
...
void
meta_shaped_texture_invalidate_size (MetaShapedTexture *stex)
                                       ...
                                       int                dest_height)
{
  ...
  priv->has_viewport_src = dest_width > 0;
  meta_shaped_texture_invalidate_size (stex);
}
