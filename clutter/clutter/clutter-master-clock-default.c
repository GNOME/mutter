...
estimate_next_presentation_time (ClutterMasterClockDefault *master_clock)
{
  ...
  /* Without accurate presentation times, frame_phase=0 is an adequate guess.
   * It just means latency will be suboptimal by half a frame on average. We
   * still get maximum smoothness this way...
   */
  frame_phase = 0;
                        ...
                        gpointer     user_data)
{
  ...
  if (smooth_tick <= master_clock->prev_tick)
    {
      /* This could happen in the rare case when the ideal frame_interval
       * changes, such as video mode switching or hotplugging monitors. As such
       * it is not considered a bug (unless it's happening without mode
       * switching or hotplugging).
       */
