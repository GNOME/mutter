...
meta_surface_actor_x11_dispose (GObject *object)
{
  ...
  if (self->display->x11_display != NULL)
    ...
    {
      g_warning ("MetaSurfaceActorX11 %p outlived the X11 display connection "
                 "and can't be cleanly disposed. This is most likely a leak "
                 "in your shell code, outside of mutter.", self);
    }
