  ...
  if (meta_is_stage_views_enabled ())
    {
      ...
      MetaMonitorManagerDummy *dummy_manager;

      monitor_manager = meta_backend_get_monitor_manager (backend);
      dummy_manager = META_MONITOR_MANAGER_DUMMY (monitor_manager);

      if (meta_is_stage_views_scaled () &&
          !meta_monitor_manager_dummy_use_scaled_views (dummy_manager))
        {
          GList *l;
