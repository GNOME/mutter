...
G_DEFINE_TYPE (MetaFramebufferKms, meta_framebuffer_kms, META_TYPE_FRAMEBUFFER)

void
meta_framebuffer_kms_set_gpu (MetaFramebufferKms *framebuffer_kms,
                              MetaGpuKms         *gpu_kms)
{
  g_return_if_fail (META_IS_FRAMEBUFFER_KMS (framebuffer_kms));
  ...
  framebuffer_kms->gpu_kms = META_GPU_KMS (g_object_ref (gpu_kms));
}
                                      ...
                                      struct gbm_surface *gbm_surface)
{
  g_return_if_fail (META_IS_FRAMEBUFFER_KMS (framebuffer_kms));
  g_return_if_fail (framebuffer_kms->gbm_bo == NULL);
  g_return_if_fail (framebuffer_kms->gpu_kms != NULL);
                                             ...
                                             gboolean            use_modifiers)
{
  ...
  g_return_val_if_fail (META_IS_FRAMEBUFFER_KMS (framebuffer_kms), FALSE);
  g_return_val_if_fail (framebuffer_kms->gbm_bo == NULL, FALSE);
  g_return_val_if_fail (framebuffer_kms->gbm_surface != NULL, FALSE);
  g_return_val_if_fail (framebuffer_kms->gpu_kms != NULL, FALSE);

  drm_fd = meta_gpu_kms_get_fd (framebuffer_kms->gpu_kms);
...
{
  /* Note META_IS_FRAMEBUFFER_KMS doesn't work with const pointers */
...
{
  g_return_if_fail (META_IS_FRAMEBUFFER_KMS (framebuffer_kms));
  int drm_fd = framebuffer_kms->gpu_kms ?
               meta_gpu_kms_get_fd (framebuffer_kms->gpu_kms) :
               -1;

  if (drm_fd >= 0 &&
      framebuffer_kms->fb_id != INVALID_FB_ID &&
      framebuffer_kms->gbm_bo)  /* We don't own dumb buffers, for now... */
    drmModeRmFB (drm_fd, framebuffer_kms->fb_id);
...
static void
meta_framebuffer_kms_dispose (GObject *object)
{
  MetaFramebufferKms *framebuffer_kms = META_FRAMEBUFFER_KMS (object);

  g_clear_object (&framebuffer_kms->gpu_kms);

  G_OBJECT_CLASS (meta_framebuffer_kms_parent_class)->dispose (object);
}
...
meta_framebuffer_kms_class_init (MetaFramebufferKmsClass *klass)
{
  ...
  object_class->dispose = meta_framebuffer_kms_dispose;
