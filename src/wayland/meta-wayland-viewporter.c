...
destroy_wl_viewport (struct wl_resource *resource)
{
  ...
  surface->pending->buffer_viewport.buffer.src_x = -1;
  surface->pending->buffer_viewport.surface.width = -1;
  surface->pending->buffer_viewport.surface.height = -1;
                     ...
                     wl_fixed_t src_height)
{
  ...
  if ((new_x >= 0 && new_y >= 0 &&
    ...
    {
      if (new_x != vp->buffer.src_x ||
          new_y != vp->buffer.src_y ||
          new_width != vp->buffer.src_width ||
          new_height != vp->buffer.src_height)
        {
          pending_vp->buffer.src_x = new_x;
          pending_vp->buffer.src_y = new_y;
          pending_vp->buffer.src_width = new_width;
          pending_vp->buffer.src_height = new_height;
