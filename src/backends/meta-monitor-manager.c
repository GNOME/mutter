                   ...
                   MetaMonitor        *monitor)
{
  ...
  const char *output_vendor_name = NULL;
  ...
  output_vendor_name = meta_monitor_get_vendor (monitor);

  if (g_strcmp0 (output_vendor_name, "unknown") != 0)
    {
      if (!manager->pnp_ids)
      ...
      if (!vendor_name)
        vendor_name = g_strdup (output_vendor_name);
    }
  else
    {
      if (inches != NULL)
