...
static GQuark quark_instantaneous = 0;
...
clutter_actor_class_init (ClutterActorClass *klass)
{
  ...
  quark_instantaneous = g_quark_from_static_string ("instantaneous");
  ...
  if (capture)
    {
      ...
      if (event->type == CLUTTER_MOTION || event->type == CLUTTER_TOUCH_UPDATE)
        detail = quark_continuous;
      else
        detail = quark_instantaneous;
