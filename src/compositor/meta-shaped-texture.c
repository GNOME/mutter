...
update_size (MetaShapedTexture *stex)
{
  ...
  if (stex->has_viewport_dst_size)
    {
      dst_width = ceil (stex->viewport_dst_width * stex->buffer_scale);
      dst_height = ceil (stex->viewport_dst_height * stex->buffer_scale);
    }
  else if (stex->has_viewport_src_rect)
    {
      dst_width = ceil (stex->viewport_src_rect.size.width * stex->buffer_scale);
      dst_height = ceil (stex->viewport_src_rect.size.height * stex->buffer_scale);
    }
  else
    {
      ...
      if (meta_monitor_transform_is_rotated (stex->transform))
        {
          cogl_matrix_scale (&matrix,
                             ...
                             (stex->tex_height / stex->buffer_scale),
                             stex->viewport_src_rect.size.height /
                             (stex->tex_width / stex->buffer_scale),
                             1);
        }
      else
        {
