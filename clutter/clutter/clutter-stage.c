...
struct _ClutterStagePrivate
{
  ...
  GQueue *event_queue;
  guint event_flush_source;
  int64_t last_event_flush_time;
...
_clutter_stage_flush_events (gpointer user_data)
{
  ...
  priv->event_flush_source = 0;
  priv->last_event_flush_time = g_get_monotonic_time ();
                            ...
                            gboolean      copy_event)
{
  ...
  if (!priv->event_flush_source)
    {
      int64_t now = g_get_monotonic_time ();
      int64_t next_redraw = _clutter_stage_get_update_time (stage);
      int64_t refresh_interval = _clutter_stage_get_refresh_interval (stage);
      int64_t flush_time;
      guint timeout_ms;

      /* Limit the event rate to be the refresh rate */
      flush_time = priv->last_event_flush_time + refresh_interval;

      /* And if a redraw time is known then use that to further correct the
       * phase to stay precisely half a frame out of phase with redraws.
       */
      if (next_redraw >= 0 && flush_time >= next_redraw)
        flush_time = next_redraw + refresh_interval / 2;

      timeout_ms = (flush_time > now)
                 ? (flush_time - now) / 1000
                 : 0;

      priv->event_flush_source =
...
clutter_stage_finalize (GObject *object)
{
  ...
  if (priv->event_flush_source)
    g_source_remove (priv->event_flush_source);
