                                    ...
                                    gint                sync_delay)
{
  ...
  int64_t min_render_allowance, max_render_allowance;
  int64_t earliest_update_time, earliest_presentation_time;
  ...
  min_render_allowance = refresh_interval / 2;
  max_render_allowance = refresh_interval - 1000 * sync_delay;

  if (min_render_allowance > max_render_allowance)
    min_render_allowance = max_render_allowance;

  earliest_presentation_time = now + min_render_allowance;
  earliest_update_time = earliest_presentation_time - max_render_allowance;

  while (stage_cogl->update_time < earliest_update_time)
