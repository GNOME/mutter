...
static void
_clear_weak_pointer_list (GList **list)
{
  GList *elem;
...
static void
_clutter_stage_clear_pending_queue_relayouts (ClutterStage *stage)
{
  ClutterStagePrivate *priv = stage->priv;

  _clear_weak_pointer_list (&priv->pending_queue_relayouts);
}
  ...
  if (priv->pending_queue_relayouts)
    {
      ...
      if (first == (ClutterActor *) stage)
        ...
        _clutter_stage_clear_pending_queue_relayouts (stage);
    }
  else
    {
      _clutter_stage_schedule_update (stage);
...
_clutter_stage_maybe_relayout (ClutterActor *actor)
{
  ...
  for (elem = stolen_list; elem != NULL; elem = elem->next)
    {
      ...
      if (!elem->data)  /* weak reference destroyed? */
        continue;

      entry = CLUTTER_ACTOR (elem->data);
  ...
  _clear_weak_pointer_list (&stolen_list);
