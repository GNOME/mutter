             ...
             MetaUIFrame   *frame)
{
  ...
  if (meta_is_wayland_compositor ())
    gdk_display_sync (frames->x11_display->gdk_display);

  meta_window_set_frame_redraw_pending (window, FALSE);

  g_signal_handlers_disconnect_by_func (gdk_window_get_frame_clock (frame->window),
                  ...
                  cairo_t   *cr)
{
  ...
  g_signal_connect (gdk_window_get_frame_clock (frame->window),
                    "after-paint",
                    G_CALLBACK (after_paint),
                    frame);
