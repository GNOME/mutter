  ...
  if (_clutter_context_get_show_fps ())
    {
      ...
      priv->perf_cumulative_draw_time += (now - start_time);
      ...
      if (priv->perf_frame_count && priv->perf_last_print_time)
        {
          ...
          if (print_interval >= G_USEC_PER_SEC)
            {
              gint avg_fps_x10 = priv->perf_frame_count * 10 * G_USEC_PER_SEC /
                                 print_interval;
              gint avg_draw_time_ms_x10 = priv->perf_cumulative_draw_time * 10 /
                                          (priv->perf_frame_count * 1000);
              gint64 avg_frame_interval = print_interval /
                                          priv->perf_frame_count;
              gint64 avg_draw_interval = priv->perf_sum_draw_intervals /
