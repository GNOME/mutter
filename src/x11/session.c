...
static void
save_phase_2_callback (SmcConn   smc_conn,
                       SmPointer client_data)
{
  gboolean shutdown;
...
static void
save_complete_callback (SmcConn   smc_conn,
                        SmPointer client_data)
{
  /* nothing */
...
static void
shutdown_cancelled_callback (SmcConn   smc_conn,
                             SmPointer client_data)
{
...
static void
interact_callback (SmcConn   smc_conn,
                   SmPointer client_data)
{
  /* nothing */
  gboolean shutdown;
          ...
          if (meta_is_verbose ())
            {
              if (!both_null_or_matching (info->id, window->sm_client_id))
                ...
                meta_topic (META_DEBUG_SM,
                            "???? should not happen - window %s doesn't match saved state %s for no good reason\n",
                            window->desc, info->id);
            }
        }
