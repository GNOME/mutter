...
#include "compositor/meta-screen-cast-window.h"
                                     ...
                                     cairo_rectangle_int_t *bounds)
{
  ...
  bounds->x = (int) floor ((window->rect.x - buffer_rect.x) / scale_x);
  bounds->y = (int) floor ((window->rect.y - buffer_rect.y) / scale_y);
  bounds->width = (int) ceil (window->rect.width / scale_x);
  bounds->height = (int) ceil (window->rect.height / scale_y);
}
...

{
  ...
  v1.x = CLAMP (bounds.x + (float) x,
                bounds.x,
                bounds.x + bounds.width);
  v1.y = CLAMP (bounds.y + (float) y,
                bounds.y,
