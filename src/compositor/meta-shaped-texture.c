...
update_size (MetaShapedTexture *stex)
{
  ...
  if (stex->has_viewport_dst_size)
  ...
  else if (stex->has_viewport_src_rect)
    {
      ...
      dst_width = stex->viewport_src_rect.size.width * buffer_scale;
      dst_height = stex->viewport_src_rect.size.height * buffer_scale;
    }
  else
    {
      if (meta_monitor_transform_is_rotated (stex->transform))
        {
          if (stex->texture)
            {
              dst_width = stex->tex_height / stex->buffer_scale;
              dst_height = stex->tex_width / stex->buffer_scale;
            }
          else
            {
              dst_width = stex->fallback_height / stex->buffer_scale;
              dst_height = stex->fallback_width / stex->buffer_scale;
            }
        }
      else
        {
          if (stex->texture)
            {
              dst_width = stex->tex_width / stex->buffer_scale;
              dst_height = stex->tex_height / stex->buffer_scale;
            }
          else
            {
              dst_width = stex->fallback_width / stex->buffer_scale;
              dst_height = stex->fallback_height / stex->buffer_scale;
            }
        }
    }
      ...
      if (meta_monitor_transform_is_rotated (stex->transform))
        {
          cogl_matrix_scale (&matrix,
                             ...
                             (stex->tex_height / (float) stex->buffer_scale),
                             stex->viewport_src_rect.size.height /
                             (stex->tex_width / (float) stex->buffer_scale),
                             1);
        }
      else
        {
  ...
  if (stex->has_viewport_src_rect || stex->has_viewport_dst_size)
    {
      ...
      if (stex->has_viewport_dst_size)
        ...
        {
          dst_width = (float) stex->tex_width * stex->buffer_scale;
          dst_height = (float) stex->tex_height * stex->buffer_scale;
        }
