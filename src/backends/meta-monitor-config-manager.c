  ...
  for (l = config->logical_monitor_configs; l; l = l->next)
    {
      ...
      for (GList *monitor_configs = logical_monitor_config->monitor_configs;
           monitor_configs;
           monitor_configs = monitor_configs->next)
        {
          MetaMonitorConfig *monitor_config = monitor_configs->data;
          MetaMonitorSpec *monitor_spec = monitor_config->monitor_spec;
          MetaMonitor *monitor;

          monitor = meta_monitor_manager_get_monitor_from_spec (manager, monitor_spec);

          for (GList *outputs = meta_monitor_get_outputs (monitor);
               outputs;
