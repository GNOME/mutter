...
static gboolean
logical_monitors_have_common_transform (MetaCursorRenderer   *renderer,
                                        MetaCursorSprite     *cursor_sprite,
                       ...
                       GList              *gpus)
{
  ...
  if (!logical_monitors_have_common_scale (renderer, cursor_sprite, &scale) ||
      !logical_monitors_have_common_transform (renderer, cursor_sprite, &transform))
    return FALSE;

  cursor_priv = get_cursor_priv (cursor_sprite);
  if (!cursor_priv->is_cursor_image_scalable &&
...
ensure_cursor_priv (MetaCursorSprite *cursor_sprite)
{
  ...
  cursor_priv->is_cursor_image_scalable = TRUE;
                          ...
                          MetaGpuKms         *gpu_kms)
{
  ...
  if (logical_monitors_have_common_scale (renderer, cursor_sprite, &scale) &&
      scale != cursor_priv->current_relative_scale)
    return FALSE;

  if (logical_monitors_have_common_transform (renderer, cursor_sprite, &transform) &&
      transform != cursor_priv->current_relative_transform)
    return FALSE;
...
cursor_sprite_needs_scaling_or_transforming (MetaCursorRenderer   *renderer,
                                             MetaCursorSprite     *cursor_sprite,
                                             float                *relative_scale,
                                             MetaMonitorTransform *relative_transform)
{
  MetaMonitorTransform transform;
  gboolean have_common_transform;
  gboolean have_common_scale;
  float scale;

  have_common_transform =
    logical_monitors_have_common_transform (renderer, cursor_sprite, &transform);
  have_common_scale =
    logical_monitors_have_common_scale (renderer, cursor_sprite, &scale);

  *relative_transform = transform;
  *relative_scale = scale;

  return have_common_scale &&
         have_common_transform &&
         (scale != 1.0 || transform != META_MONITOR_TRANSFORM_NORMAL);
  ...
  if (shm_buffer)
    {
      ...
      if (cursor_sprite_needs_scaling_or_transforming (renderer,
                                                       cursor_sprite,
                                                       &relative_scale,
                                                       &relative_transform))
        {
          ...
          cursor_priv->current_relative_scale = relative_scale;
          cursor_priv->current_relative_transform = relative_transform;
          cursor_priv->is_cursor_image_scalable = TRUE;

          surface = scale_and_transform_cursor_sprite_cpu (buffer_data,
                                                           width,
                                                           height,
                                                           rowstride,
                                                           relative_scale,
        ...
        {
          cursor_priv->current_relative_scale = 1.0;
          cursor_priv->current_relative_transform =
            META_MONITOR_TRANSFORM_NORMAL;
          cursor_priv->is_cursor_image_scalable = TRUE;
  ...
  else
    {
      ...
      cursor_priv->current_relative_scale = 1.0;
      cursor_priv->current_relative_transform = META_MONITOR_TRANSFORM_NORMAL;
      cursor_priv->is_cursor_image_scalable = FALSE;
                                            ...
                                            MetaCursorSpriteXcursor *sprite_xcursor)
{
  ...
  if (cursor_sprite_needs_scaling_or_transforming (renderer,
                                                   ...
                                                   &relative_transform))
    {
      ...
      surface = scale_and_transform_cursor_sprite_cpu ((uint8_t *) xc_image->pixels,
                                                       xc_image->width,
                                                       xc_image->height,
                                                       xc_image->width * 4,
                                                       relative_scale,
