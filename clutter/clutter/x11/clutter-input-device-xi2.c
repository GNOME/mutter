...
clutter_input_device_xi2_finalize (GObject *object)
{
  ...
  if (device_xi2->idle_query_timer)
    g_source_remove (device_xi2->idle_query_timer);
...
static gboolean
trigger_idle_query_timer (gpointer data)
{
  ClutterInputDeviceXI2 *device_xi2 = CLUTTER_INPUT_DEVICE_XI2 (data);

  device_xi2->idle_query_timer = 0;
...

{
  ...
  if (device_xi2->idle_query_timer == 0)
    {
      device_xi2->query_status = clutter_input_device_xi2_query_pointer_location (device_xi2);
      device_xi2->idle_query_timer = clutter_threads_add_idle (trigger_idle_query_timer, device_xi2);
    }
