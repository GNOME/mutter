                            ...
                            GError                   **error)
{
  ...
  if (drmModeObjectSetProperty (fd,
                                meta_kms_connector_get_id (connector),
                                DRM_MODE_OBJECT_CONNECTOR,
                                connector_property->prop_id,
                                connector_property->value) != 0)
    {
      g_set_error (error, G_IO_ERROR, g_io_error_from_errno (errno),
                   "Failed to set connector property: %s", g_strerror (errno));
      return FALSE;
    }
                        ...
                        GError          **error)
{
  ...
  if (drmModeObjectSetProperty (fd,
    ...
    {
      g_set_error (error, G_IO_ERROR, g_io_error_from_errno (errno),
                   "Failed to set plane property: %s", g_strerror (errno));
      return FALSE;
    }
                  ...
                  GError         **error)
{
      ...
      if (!plane_assignment)
        {
          g_set_error (error, G_IO_ERROR, G_IO_ERROR_FAILED,
                       "Missing primary plane assignment for legacy mode set");
          return FALSE;
        }
  ...
  if (drmModeSetCrtc (fd,
    ...
    {
      g_set_error (error, G_IO_ERROR, g_io_error_from_errno (errno),
                   "Failed to set CRTC mode: %s", g_strerror (errno));
      return FALSE;
    }
                          ...
                          MetaKmsPageFlipData *page_flip_data)
{
  ...
  retry_page_flip_data->refresh_rate = refresh_rate;
...
static void
invoke_and_clear_page_flip_datas (GList                        **page_flip_datas,
...
mode_set_fallback_feedback_idle (gpointer user_data)
{
  ...
  if (!impl_simple->pending_page_flip_retries)
    ...
    {
      invoke_and_clear_page_flip_datas (
                   ...
                   GError                 **error)
{
  ...
  if (ret != 0)
    {
      g_set_error (error, G_IO_ERROR, g_io_error_from_errno (-ret),
                   "drmModeSetCrtc failed: %s", g_strerror (-ret));
      return FALSE;
    }
