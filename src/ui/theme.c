  ...
  if (opacity > 0.0)
    {
      if (opacity != 1.0)
        {
          double *saved_opacity;
          cairo_push_group (cr);
          saved_opacity = g_new0 (double, 1);
                                   ...
                                   cairo_surface_t         *mini_icon)
{
   ...
   *  - GTK now expects the device scale to be set and match the final scaling
   *    or the surface caching won't take this in account breaking -gtk-scaled
   *    items.
   */
