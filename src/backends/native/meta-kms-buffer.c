                              ...
                              GError             **error)
{
  ...
  kms_buffer->gbm.gpu_kms = META_GPU_KMS (g_object_ref (gpu_kms));
...
meta_kms_buffer_dispose (GObject *object)
{
  ...
  if (kms_buffer->type == META_KMS_BUFFER_TYPE_GBM &&
      kms_buffer->gbm.gpu_kms != NULL)
    {
      if (kms_buffer->gbm.fb_id != INVALID_FB_ID)
        {
          int drm_fd = meta_gpu_kms_get_fd (kms_buffer->gbm.gpu_kms);

          /* This might sound like a finalize thing but it needs to happen
           * before the below clear, so must happen in dispose.
           */
          drmModeRmFB (drm_fd, kms_buffer->fb_id);
          kms_buffer->fb_id = INVALID_FB_ID;
        }

      g_clear_object (&kms_buffer->gbm.gpu_kms);
    }
