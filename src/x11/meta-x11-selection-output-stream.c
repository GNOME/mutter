                                                   ...
                                                   gboolean                      success)
{
  ...
  meta_x11_error_trap_push (priv->display);
  ...
  meta_x11_error_trap_pop (priv->display);
}
...
{
  MetaX11SelectionOutputStreamPrivate *priv = meta_x11_selection_output_stream_get_instance_private (stream);
  ...
  return priv->data->len >= get_max_request_size (priv->display);
}
  ...
  if (!g_output_stream_is_closing (G_OUTPUT_STREAM (stream)))
    {
      ...
      XChangeProperty (xdisplay,
                       priv->xwindow,
                       priv->xproperty,
                       XInternAtom (priv->display->xdisplay, "INCR", True),
                       32,
                       PropModeReplace,
...
{
  MetaX11SelectionOutputStream *stream = META_X11_SELECTION_OUTPUT_STREAM (data);
...
meta_x11_selection_output_stream_write (GOutputStream  *output_stream,
                                        ...
                                        GError        **error)
{
  MetaX11SelectionOutputStream *stream = META_X11_SELECTION_OUTPUT_STREAM (output_stream);
  MetaX11SelectionOutputStreamPrivate *priv = meta_x11_selection_output_stream_get_instance_private (stream);
                                               ...
                                               GError        **error)
{
  ...
  g_return_val_if_fail (g_task_get_source_tag (G_TASK (result)) == meta_x11_selection_output_stream_write_async, -1);
...
meta_x11_selection_output_stream_invoke_close (gpointer stream)
{
  ...
  priv->display->selection.output_streams =
    g_list_remove (priv->display->selection.output_streams, stream);
...
{
  MetaX11SelectionOutputStream *stream = META_X11_SELECTION_OUTPUT_STREAM (object);
  MetaX11SelectionOutputStreamPrivate *priv = meta_x11_selection_output_stream_get_instance_private (stream);
                                         ...
                                         const XEvent                 *xevent)
{
  MetaX11SelectionOutputStreamPrivate *priv =
    meta_x11_selection_output_stream_get_instance_private (stream);
  Display *xdisplay = priv->display->xdisplay;
...
GOutputStream *
meta_x11_selection_output_stream_new (MetaX11Display                *display,
                                      Window                         requestor,
                                      ...
                                      gulong                         timestamp)
{
  ...
  display->selection.output_streams =
    g_list_prepend (display->selection.output_streams, stream);

  priv->display = display;
  priv->xwindow = requestor;
  priv->selection = g_strdup (selection);
  priv->xselection = XInternAtom (display->xdisplay, priv->selection, False);
  priv->target = g_strdup (target);
  priv->xtarget = XInternAtom (display->xdisplay, priv->target, False);
  priv->property = g_strdup (property);
  priv->xproperty = XInternAtom (display->xdisplay, priv->property, False);
  priv->type = g_strdup (type);
  priv->xtype = XInternAtom (display->xdisplay, priv->type, False);
  priv->format = format;
