...
{
  MetaX11SelectionInputStreamPrivate *priv = meta_x11_selection_input_stream_get_instance_private (stream);
...
meta_x11_selection_input_stream_flush (MetaX11SelectionInputStream *stream)
{
  ...
  written = meta_x11_selection_input_stream_fill_buffer (stream, priv->pending_data, priv->pending_size);
...
meta_x11_selection_input_stream_complete (MetaX11SelectionInputStream *stream)
{
  ...
  priv->display->selection.input_streams =
    g_list_remove (priv->display->selection.input_streams, stream);
...
meta_x11_selection_input_stream_read (GInputStream  *input_stream,
                                      ...
                                      GError       **error)
{
  MetaX11SelectionInputStream *stream = META_X11_SELECTION_INPUT_STREAM (input_stream);
                                             ...
                                             GError       **error)
{
  ...
  g_return_val_if_fail (g_task_get_source_tag (G_TASK (result)) == meta_x11_selection_input_stream_read_async, -1);
...
{
  MetaX11SelectionInputStream *stream = META_X11_SELECTION_INPUT_STREAM (object);
  MetaX11SelectionInputStreamPrivate *priv = meta_x11_selection_input_stream_get_instance_private (stream);
        ...
        if (xevent->xselection.property == None)
        ...
        else
          {
            ...
            if (bytes == NULL)
              ...
              {
                if (priv->xtype == XInternAtom (priv->display->xdisplay, "INCR", False))
                  {
...
void
meta_x11_selection_input_stream_new_async (MetaX11Display      *display,
                                           Window               window,
                                           ...
                                           gpointer             user_data)
{
  ...
  display->selection.input_streams =
    g_list_prepend (display->selection.input_streams, stream);
  ...
  XConvertSelection (display->xdisplay,
                     priv->xselection,
                     priv->xtarget,
                     priv->xproperty,
                     window,
                                            ...
                                            GError       **error)
{
  ...
  task = G_TASK (result);
  g_return_val_if_fail (g_task_get_source_tag (task) == meta_x11_selection_input_stream_new_async, NULL);
