...
static gboolean
close_display_later (gpointer user_data)
{
                         ...
                         XEvent         *event)
{
  ...
  // Closing a GdkDisplay during an event handler is not currently supported
  // by Gdk and it will result in a crash when doing 'mutter --replace'. Using
  // an idle function will close it safely in a subsequent main loop iteration.
  if (!x11_display->display_close_idle)
    {
      x11_display->xselectionclear_timestamp = event->xselectionclear.time;
      x11_display->display_close_idle = g_idle_add (close_display_later, x11_display);
    }
