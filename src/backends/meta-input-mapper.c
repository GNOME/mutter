...
struct _MetaInputMapper
{
  ...
  GHashTable *output_devices; /* MetaOutput -> MetaMapperOutputInfo */
};
...
struct _MetaMapperOutputInfo
{
  ...
  GList *input_devices;
  MetaInputCapabilityFlags flags;
};
...
mapper_output_info_new (MetaLogicalMonitor *monitor)
{
                              ...
                              MetaMonitor          *monitor)
{
  ...
  g_signal_emit (input->mapper, signals[DEVICE_MAPPED], 0,
                 input->device,
                 output ? output->monitor : NULL, monitor);
}
                              ...
                              MetaMonitor          *monitor)
{
  ...
  output->flags |= mapper_input_info_get_caps (input);
                                 ...
                                 MetaMapperInputInfo  *input)
{
  ...
  for (l = output->input_devices; l; l = l->next)
    output->flags |= mapper_input_info_get_caps (l->data);
...
static gboolean
mapper_output_info_check_add_input (MetaMapperOutputInfo *output,
                                    MetaMapperInputInfo  *input,
                                    MetaMonitor          *monitor)
{
  /* Output already has a device with the same capabilities */
  if (output->flags & mapper_input_info_get_caps (input))
    return FALSE;

  mapper_output_info_add_input (output, input, monitor);
  return TRUE;
}
...
static void
mapping_helper_unset (MappingHelper *helper)
{
  ...
  for (l = logical_monitors; l; l = l->next)
    {
      ...
      if (!g_list_find (logical_monitor->monitors, panel))
        continue;
...
{
  guint best = N_OUTPUT_MATCHES;
  GList *logical_monitors, *l;
  MetaLogicalMonitor *match = NULL;
  ...
  for (l = logical_monitors; l; l = l->next)
    {
      ...
      if (match_edid (input, l->data, &matched_monitor, &edid_match))
        {
          best = MIN (best, edid_match);
          info->candidates[edid_match].logical_monitor = l->data;
          info->candidates[edid_match].monitor = matched_monitor;
        }
    }

  if (find_size_match (input, logical_monitors, &match, &matched_monitor))
    {
      best = MIN (best, META_MATCH_SIZE);
      info->candidates[META_MATCH_SIZE].logical_monitor = match;
      info->candidates[META_MATCH_SIZE].monitor = matched_monitor;
    }

  if (input->builtin)
    {
      best = MIN (best, META_MATCH_IS_BUILTIN);
      find_builtin_output (mapper, logical_monitors,
                           &info->candidates[META_MATCH_IS_BUILTIN].logical_monitor,
                           &info->candidates[META_MATCH_IS_BUILTIN].monitor);
    }
  ...
  for (i = 0; i < helper->device_maps->len; i++)
    {
      ...
      for (j = 0; j < N_OUTPUT_MATCHES; j++)
        {
          if (!info->candidates[j].logical_monitor)
            continue;

          output = g_hash_table_lookup (mapper->output_devices,
                                        info->candidates[j].logical_monitor);
          if (!output)
            continue;

          if (mapper_output_info_check_add_input (output, info->input,
                                                  info->candidates[j].monitor))
            break;
        }
    }
}
  ...
  for (l = logical_monitors; l; l = l->next)
    {
      ...
      info = mapper_output_info_new (monitor);
      g_hash_table_insert (mapper->output_devices, monitor, info);
    }
