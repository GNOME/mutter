                            ...
                            GError             **error)
{
  ...
  if (priv->has_hw_cursor &&
                                 ...
                                 NULL))
    {
      ...
      cursor_rect = (ClutterRect) {
        .origin = {
          .x = crtc_cursor_x,
          .y = crtc_cursor_y,
        },
        .size = {
          .width = tex_width * cursor_crtc_scale,
          .height = tex_height * cursor_crtc_scale,
        }
      };

      inverted_transform = meta_monitor_transform_invert (transform);
      meta_clutter_rect_transform (&cursor_rect,
                                   inverted_transform,
      ...
      drmModeMoveCursor (kms_fd,
                         crtc->crtc_id,
                         floorf (cursor_rect.origin.x),
                         floorf (cursor_rect.origin.y));
...
{
  MetaMonitorTransform transform;
  gboolean has_common_transform;
  gboolean has_common_scale;
  float scale;

  has_common_transform =
    get_common_logical_monitors_transform (renderer, cursor_sprite, &transform);
  has_common_scale =
    get_common_logical_monitors_scale (renderer, cursor_sprite, &scale);

  *relative_transform = transform;
  *relative_scale = scale;

  return (has_common_scale &&
          has_common_transform &&
          (scale != 1.0 || transform != META_MONITOR_TRANSFORM_NORMAL));
}
  ...
  if (shm_buffer)
    {
      ...
      cursor_sprite_needs_scaling_or_transforming (renderer,
                                                   cursor_sprite,
                                                   &relative_scale,
                                                   &relative_transform);
