...
clutter_offscreen_effect_pre_paint (ClutterEffect *effect)
{
  ...
  /* Save the original viewport for calculating priv->offset */
  ...
  /* Now save the global offset of the effect (not just of the actor).
   * It doesn't appear anyone actually uses this yet, but get_target_rect is
   * documented as returning it. So we should...
   */
  _clutter_util_fully_transform_vertices (&priv->last_matrix_drawn,
                                          &projection,
                                          viewport,
                                          &local_offset,
                                          &priv->offset,
