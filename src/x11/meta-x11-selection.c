...
#include "meta-x11-selection-private.h"
#include "meta-x11-selection-source-private.h"

static gboolean
atom_to_selection (Display           *xdisplay,
                   Atom               selection,
...
static gboolean
meta_x11_selection_handle_selection_request (MetaX11Display *display,
                                             XEvent         *xevent)
{
  ...
  if (!atom_to_selection (display->xdisplay, event->selection, &selection_type))
    return FALSE;
  if (display->selection.window != event->owner)
    return FALSE;
  ...
  if (event->target == gdk_x11_get_xatom_by_name ("TARGETS"))
    {
      ...
      output = meta_x11_selection_output_stream_new (display, event->requestor,
                                                     gdk_x11_get_xatom_name (event->selection),
                                                     gdk_x11_get_xatom_name (event->target),
                                                     gdk_x11_get_xatom_name (event->property),
                                                     "ATOM", 32, event->time);

      bytes = mimetypes_to_bytes (mimetypes, display->xdisplay);
  ...
  else
    {
      ...
      if (has_target)
        {
          output = meta_x11_selection_output_stream_new (display,
                                                         event->requestor,
                                                         gdk_x11_get_xatom_name (event->selection),
                                                         NULL,
                                                         gdk_x11_get_xatom_name (event->property),
               ...
               gpointer      user_data)
{
  ...
  MetaX11Display *display = data->display;
  GError *error = NULL;

  source = meta_x11_selection_source_new_finish (res, &error);
  if (source)
    {
      meta_selection_set_owner (selection, selection_type, source);
      g_set_object (&display->selection.owners[selection_type], source);
    }
  else
    {
...
static gboolean
meta_x11_selection_handle_xfixes_selection_notify (MetaX11Display *display,
                                                   XEvent         *xevent)
{
  ...
  if (!atom_to_selection (xdisplay, event->selection, &selection_type))
    return FALSE;
  ...
  if (event->owner == None)
    {
      if (display->selection.owners[selection_type])
        {

          /* An X client went away, clear the selection */
          meta_selection_unset_owner (selection, selection_type,
                                      display->selection.owners[selection_type]);
        }
    }
  else if (event->owner != display->selection.window)
    {
      SourceNewData *data = g_new (SourceNewData, 1);

      data->display = display;
      data->selection = selection;
      data->selection_type = selection_type;

      meta_x11_selection_source_new_async (display,
                                           event->owner,
                                           event->timestamp,
                                           event->selection,
                                           NULL,
...
gboolean
meta_x11_selection_handle_event (MetaX11Display *display,
                                 XEvent         *xevent)
{
  if (xevent->type == SelectionRequest)
    return meta_x11_selection_handle_selection_request (display, xevent);
  else if (xevent->type - display->xfixes_event_base == XFixesSelectionNotify)
    return meta_x11_selection_handle_xfixes_selection_notify (display, xevent);
                  ...
                  MetaX11Display      *display)
{
  ...
  if (owner && !META_IS_X11_SELECTION_SOURCE (owner))
    {
