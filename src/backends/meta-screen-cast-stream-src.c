...
meta_screen_cast_stream_src_maybe_record_frame (MetaScreenCastStreamSrc *src)
{
  ...
  if ((mc = spa_buffer_find_meta (spa_buffer, priv->pipewire_type->meta.VideoCrop)))
    {
      if (meta_screen_cast_stream_src_get_videocrop (src, &crop_rect))
        {
          mc->x = crop_rect.x;
          mc->y = crop_rect.y;
          mc->width = crop_rect.width;
          mc->height = crop_rect.height;
        }
      else
        {
          mc->x = 0;
          mc->y = 0;
          mc->width = priv->stream_width;
          mc->height = priv->stream_height;
        }
    }
