                                        ...
                                        MetaMonitorManager    *monitor_manager)
{
  ...
  new_table = g_hash_table_new_full (g_int64_hash, g_int64_equal, g_free,
  ...
  for (l = logical_monitors; l; l = l->next)
    {
      ...
      g_hash_table_lookup_extended (compositor->outputs,
      ...
      if (wayland_output)
      ...
      else
        {
          ...
          keyptr = g_new (uint64_t, 1);
          *keyptr = logical_monitor->winsys_id;
        }
...
meta_wayland_outputs_init (MetaWaylandCompositor *compositor)
{
  ...
  compositor->outputs = g_hash_table_new_full (g_int64_hash, g_int64_equal, g_free,
                                               wayland_output_destroy_notify);
  compositor->outputs = meta_wayland_compositor_update_outputs (compositor, monitors);
