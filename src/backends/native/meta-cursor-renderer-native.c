...
typedef struct _MetaCursorNativePrivate
{
  ...
  gboolean cursor_image_scalable;
...
static gboolean
get_cursor_image_scalable (MetaCursorNativePrivate *cursor_priv);
...
static gboolean
has_distinct_monitor_scale (MetaCursorRenderer   *renderer,
                            MetaCursorSprite     *cursor_sprite,
                            float                *scale_out)
{
  MetaCursorRendererNative *cursor_renderer_native =
...
static gboolean
has_distinct_monitor_transform (MetaCursorRenderer   *renderer,
                                MetaCursorSprite     *cursor_sprite,
                       ...
                       GList              *gpus)
{
  ...
  if (!has_distinct_monitor_scale (renderer, cursor_sprite, &scale) ||
      !has_distinct_monitor_transform (renderer, cursor_sprite, &transform))
    return FALSE;

  cursor_priv = get_cursor_priv (cursor_sprite);
  if (!get_cursor_image_scalable (cursor_priv) &&
...
static void
set_cursor_image_scalable (MetaCursorNativePrivate *cursor_priv,
                           gboolean                 scalable)
{
  cursor_priv->cursor_image_scalable = scalable;
}

static gboolean
get_cursor_image_scalable (MetaCursorNativePrivate *cursor_priv)
{
  return cursor_priv->cursor_image_scalable;
}
...
ensure_cursor_priv (MetaCursorSprite *cursor_sprite)
{
  ...
  set_cursor_image_scalable (cursor_priv, TRUE);
                          ...
                          MetaGpuKms         *gpu_kms)
{
  ...
  if (has_distinct_monitor_transform (renderer, cursor_sprite, &transform) &&
  ...
  if (shm_buffer)
    {
      ...
      if ((has_distinct_monitor_scale (renderer, cursor_sprite, &relative_scale) &&
           has_distinct_monitor_transform (renderer, cursor_sprite, &relative_transform)) &&
           (relative_scale != 1.0 ||
  ...
  else
    {
      ...
      set_current_relative_transform (cursor_priv,
                                      META_MONITOR_TRANSFORM_NORMAL);
      set_cursor_image_scalable (cursor_priv, FALSE);
