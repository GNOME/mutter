/*
 * Copyright (C) 2007-2010 Gerd Kohlberger <gerdko gmail com>
...
is_secondary_click_enabled (ClutterInputDevice *device)
{
  ...
  clutter_device_manager_get_pointer_a11y_settings (device->device_manager,
                                                    &settings);
...
static gboolean
is_pointer_accessibility_enabled (ClutterInputDevice *device)
{
...
pointer_has_moved (ClutterInputDevice *device)
{
  ...
  dx = device->ptr_a11y_data->dwell_x - point.x;
  dy = device->ptr_a11y_data->dwell_y - point.y;
...
get_dwell_direction (ClutterInputDevice *device)
{
  ...
  dx = ABS (device->ptr_a11y_data->dwell_x - point.x);
  dy = ABS (device->ptr_a11y_data->dwell_y - point.y);

  /* The pointer hasn't moved */
  if (!pointer_has_moved (device))
    return CLUTTER_A11Y_DWELL_DIRECTION_NONE;

  if (device->ptr_a11y_data->dwell_x < point.x)
    {
      if (dx > dy)
  ...
  if (device->ptr_a11y_data->dwell_y < point.y)
...
update_dwell_position (ClutterInputDevice *device)
{
  ...
  clutter_input_device_get_coords (device, NULL, &point);
  device->ptr_a11y_data->dwell_x = point.x;
  device->ptr_a11y_data->dwell_y = point.y;
}
...
_clutter_input_pointer_a11y_on_motion_event (ClutterInputDevice *device)
{
  ...
  if (!is_pointer_accessibility_enabled (device))
    return;
