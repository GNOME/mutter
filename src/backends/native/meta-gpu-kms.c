                     ...
                     int64_t     page_flip_time_ns)
{
  ...
  g_value_set_boolean (&params[3], TRUE);
  g_value_init (&params[4], G_TYPE_INT64);
  g_value_set_int64 (&params[4], page_flip_time_ns);
  g_closure_invoke (flip_closure, NULL, 5, params, NULL);
  g_closure_unref (flip_closure);
}

static void
invoke_dropped_flip_closure (GClosure   *flip_closure,
                             MetaGpuKms *gpu_kms,
                             MetaCrtc   *crtc)
{
  GValue params[] = {
  ...
  g_value_set_object (&params[2], crtc);
  g_value_init (&params[3], G_TYPE_BOOLEAN);
  g_value_set_boolean (&params[3], FALSE);
  g_value_init (&params[4], G_TYPE_INT64);
  g_closure_invoke (flip_closure, NULL, 5, params, NULL);
  g_closure_unref (flip_closure);
}
                              ...
                              MetaKmsBuffer *kms_fb)
{
  ...
  uint32_t fb_id = kms_fb ? meta_kms_buffer_get_fb_id (kms_fb) : 0;
  ...
  if (scanouts->next_closure)
    {
      invoke_dropped_flip_closure (scanouts->next_closure, gpu_kms, crtc);
...
{
  g_clear_object (&closure_container->crtc);
                        ...
                        gboolean      *fb_in_use)
{
  ...
  if (!gpu_kms->page_flips_not_supported)
    {
      ...
      ret = drmModePageFlip (kms_fd,
                             crtc->crtc_id,
                             meta_kms_buffer_get_fb_id (kms_fb),
  ...
  if (ret != 0)
    {
      if (meta_gpu_kms_apply_crtc_mode (gpu_kms, crtc, x, y, kms_fb))
        {
          *fb_in_use = TRUE;
          return FALSE;
        }
  ...
  if (crtc)
    {
      ...
      if (scanouts->next)
        {
          gboolean fb_in_use;
          MetaKmsBuffer *next_fb = scanouts->next;
