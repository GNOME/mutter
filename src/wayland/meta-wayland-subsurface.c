...
meta_wayland_subsurface_parent_state_applied (MetaWaylandSubsurface *subsurface)
{
      ...
      for (it = surface->sub.pending_placement_ops; it; it = it->next)
        {
          ...
          if (is_child (surface, sibling))
            sibling_node = sibling->subsurface_order_node;
          else
            sibling_node = sibling->subsurface_node;

          g_node_unlink (surface->subsurface_node);

          switch (op->placement)
            {
            case META_WAYLAND_SUBSURFACE_PLACEMENT_ABOVE:
              g_node_insert_after (parent->subsurface_node,
                                   sibling_node,
                                   surface->subsurface_node);
              break;
            case META_WAYLAND_SUBSURFACE_PLACEMENT_BELOW:
              g_node_insert_before (parent->subsurface_node,
                                    sibling_node,
                                    surface->subsurface_node);
              break;
            }
  ...
  meta_window_actor_wayland_rebuild_surface_tree (meta_window_actor_wayland_from_surface (surface));
                                        ...
                                        MetaRectangle         *out_geometry)
{
  ...
  for (n = g_node_first_child (surface->subsurface_node); n; n = g_node_next_sibling (n))
    {
                                 ...
                                 struct wl_resource *parent_resource)
{
  ...
  meta_wayland_surface_ensure_subsurface_node (surface);
  meta_wayland_surface_ensure_subsurface_node (parent);
  g_node_append (parent->subsurface_node, surface->subsurface_node);
