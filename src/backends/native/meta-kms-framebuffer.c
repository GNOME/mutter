...
static void
meta_kms_framebuffer_set_gpu (MetaKmsFramebuffer *kms_framebuffer,
                              MetaGpuKms         *gpu_kms)
{
  g_return_if_fail (META_IS_KMS_FRAMEBUFFER (kms_framebuffer));
  g_return_if_fail (META_IS_GPU_KMS (gpu_kms));
  g_return_if_fail (kms_framebuffer->gbm_bo == NULL);
  g_return_if_fail (kms_framebuffer->gbm_surface == NULL);

  kms_framebuffer->gpu_kms = META_GPU_KMS (g_object_ref (gpu_kms));
}
                                   ...
                                   gboolean            use_modifiers)
{
  ...
  meta_kms_framebuffer_set_gpu (kms_fb, gpu_kms);
  kms_fb->gbm_surface = gbm_surface;
  if (!meta_kms_framebuffer_acquire_swapped_buffer (kms_fb, use_modifiers))
...
void
meta_kms_framebuffer_release_buffer (MetaKmsFramebuffer *kms_framebuffer)
{
  g_return_if_fail (META_IS_KMS_FRAMEBUFFER (kms_framebuffer));
...
meta_kms_framebuffer_finalize (GObject *object)
{
  ...
  meta_kms_framebuffer_release_buffer (kms_framebuffer);
