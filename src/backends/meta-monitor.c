                                ...
                                MetaMonitorManager *manager)
{
  ...
  pnp_id = meta_monitor_get_vendor (monitor);

  if (g_strcmp0 (pnp_id, "unknown") != 0)
    {
      vendor = meta_monitor_manager_get_vendor_name_from_pnp_id (manager,
                                                                 pnp_id);

      if (!vendor)
        vendor = g_strdup (pnp_id);
    }
  else
    {
      if (inches != NULL)
        vendor = g_strdup (_("Unknown"));
      else
        vendor = g_strdup (_("Unknown Display"));
    }
