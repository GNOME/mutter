...
struct _MetaScreenCastWindowStreamSrc
{
  ...
  gulong actor_painted_handler_id;
...
static int
get_width (MetaScreenCastWindowStreamSrc *window_src)
{
  MetaScreenCastWindowStream *window_stream;
...
static int
get_height (MetaScreenCastWindowStreamSrc *window_src)
{
  MetaScreenCastWindowStream *window_stream;
              ...
              uint8_t                       *data)
{
  ...
  width = get_width (window_src);
  height = get_height (window_src);

  meta_window_get_buffer_rect (window, &buffer_rect);
  clip.x = window->rect.x - buffer_rect.x;
  clip.y = window->rect.y - buffer_rect.y;
  clip.width = width;
  clip.height = height;
      ...
      for (int i = 0; i < cr_height; i++)
        {
          ...
          if (cr_width < width)
            bzero (dst + cr_stride, (width * bpp) - cr_stride);

          src += cr_stride;
          dst += width * bpp;
        }

      for (int i = cr_height; i < height; i++)
        {
          bzero (dst, width * bpp);
          dst += width * bpp;
        }
