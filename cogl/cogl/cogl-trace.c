  ...
  if (fd != -1)
    {
      ...
      writer = sp_capture_writer_new_from_fd (fd, BUFFER_LENGTH);
    }
  else if (filename != NULL)
    {
      g_debug ("Initializing trace context with filename='%s'", filename);
      writer = sp_capture_writer_new (filename, BUFFER_LENGTH);
    }
  else
    {
      g_debug ("Initializing trace context with default dilename");
      writer = sp_capture_writer_new (COGL_TRACE_OUTPUT_FILE, BUFFER_LENGTH);
    }
