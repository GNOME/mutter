  ...
  if (stex->has_viewport_dst_size)
    {
      ...
      dst_width = round (stex->viewport_dst_width / tex_scale);
      dst_height = round (stex->viewport_dst_height / tex_scale);
    }
  else if (stex->has_viewport_src_rect)
    {
      ...
      clutter_actor_get_scale (actor, &tex_scale, NULL);
      dst_width = round (stex->viewport_src_rect.size.width / tex_scale);
      dst_height = round (stex->viewport_src_rect.size.height / tex_scale);
    }
  else
    {
      ...
      if (stex->has_viewport_src_rect)
        {
          inverted_viewport = (ClutterRect) {
            .origin.x = -stex->viewport_src_rect.origin.x,
            .origin.y = -stex->viewport_src_rect.origin.y
          };
          inverted_dst_width = stex->viewport_src_rect.size.width;
          inverted_dst_height = stex->viewport_src_rect.size.height;
        }
      else
        {
          if (stex->texture)
            {
              inverted_dst_width = round (stex->tex_width * tex_scale);
              inverted_dst_height = round (stex->tex_height * tex_scale);
            }
          else
            {
              inverted_dst_width = round (stex->fallback_width * tex_scale);
              inverted_dst_height = round (stex->fallback_height * tex_scale);
            }
        }

      if (stex->has_viewport_dst_size)
        {
          inverted_viewport.size.width = stex->viewport_dst_width;
          inverted_viewport.size.height = stex->viewport_dst_height;
        }
      else
        {
          if (stex->texture)
            {
              inverted_viewport.size.width = round (stex->tex_width * tex_scale);
              inverted_viewport.size.height = round (stex->tex_height * tex_scale);
            }
          else
            {
              inverted_viewport.size.width = round (stex->fallback_width * tex_scale);
              inverted_viewport.size.height = round (stex->fallback_height * tex_scale);
            }
        }
...
{
  if (stex->has_viewport_src_rect == FALSE ||
...
meta_shaped_texture_set_viewport_dst_size (MetaShapedTexture *stex,
                                           ...
                                           int                dst_height)
{
  if (stex->has_viewport_dst_size == FALSE ||
