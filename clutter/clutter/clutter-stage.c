  ...
  for (i = priv->pick_stack->len - 1; i >= 0; i--)
    {
      ...
      /* We +0.5f to convert from pixel-centered int coordinates to
       * pixel-boundary float coordinates.
       */
      if (rec->actor && clutter_actor_box_contains (&rec->box,
                                                    x + 0.5f,
                                                    y + 0.5f))
        return rec->actor;
    }
