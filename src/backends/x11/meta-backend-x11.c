...
xsync_user_active_alarm_set (MetaBackendX11Private *priv)
{
  ...
  GUINT64_TO_XSYNCVALUE (1, &attr.trigger.wait_value);
...
find_idletime_counter (MetaBackendX11Private *priv)
{
  ...
  counters = XSyncListSystemCounters (priv->xdisplay, &ncounters);
                     ...
                     XSyncAlarmNotifyEvent *alarm_event)
{
  ...
  attr.events = TRUE;
  XSyncChangeAlarm (priv->xdisplay, priv->user_active_alarm, XSyncCAEvents, &attr);
