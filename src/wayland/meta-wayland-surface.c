...
static void
meta_wayland_surface_transformed_coord(int width, int height,
...
meta_wayland_surface_transformed_rect(int width, int height,
                                      enum wl_output_transform transform,
                                      int scale,
                                      cairo_rectangle_int_t rect)
{
	...
	meta_wayland_surface_transformed_coord(width, height, transform, scale,
				 rect.x, rect.y, &x1, &y1);
	meta_wayland_surface_transformed_coord(width, height, transform, scale,
				 rect.x + rect.width, rect.y + rect.height, &x2, &y2);
  ...
  if (vp->buffer.src_width <= 0)
    {
      ...
      src_x = 0.0;
      src_y = 0.0;
      src_width = cogl_texture_get_width (buffer->texture) / vp->buffer.scale;
      src_height = cogl_texture_get_height (buffer->texture) / vp->buffer.scale;
      surface_width = src_width;
      surface_height = src_height;
    }
  else {
    if (vp->surface.width > 0)
      {
        surface_width = vp->surface.width;
        surface_height = vp->surface.height;
      }
    else
      {
        surface_width = cogl_texture_get_width (buffer->texture) /
                          vp->buffer.scale;
        surface_height = cogl_texture_get_height (buffer->texture) /
                          vp->buffer.scale;
      }
  ...
  for (i = 0; i < n_rects; i++)
    {
      ...
      meta_wayland_surface_surface_to_buffer_coordinate (surface,
                                                         rects[i].x,
                                                         rects[i].y,
                                                         &x1, &y1);
      ...
      rects[i] = meta_wayland_surface_transformed_rect(
                              meta_wayland_surface_get_buffer_width (surface) / vp->buffer.scale,
                              meta_wayland_surface_get_buffer_height (surface) / vp->buffer.scale,
                              vp->buffer.transform,
                              vp->buffer.scale,
                              rects[i]);
    }
...
pending_state_init (MetaWaylandPendingState *state)
{
  ...
  state->buffer_viewport.buffer.src_x = -1;
                     ...
                     MetaWaylandPendingState *to)
{
  ...
  if (from->has_new_viewport_src_rect)
    {
      to->buffer_viewport.buffer.src_x = from->buffer_viewport.buffer.src_x;
      to->buffer_viewport.buffer.src_y = from->buffer_viewport.buffer.src_y;
      to->buffer_viewport.buffer.src_width = from->buffer_viewport.buffer.src_width;
      to->buffer_viewport.buffer.src_height = from->buffer_viewport.buffer.src_height;
                                          ...
                                          MetaWaylandPendingState *pending)
{
  ...
  if (pending->has_new_viewport_src_rect)
    {
      surface->buffer_viewport.buffer.src_x = pending->buffer_viewport.buffer.src_x;
      surface->buffer_viewport.buffer.src_y = pending->buffer_viewport.buffer.src_y;
      surface->buffer_viewport.buffer.src_width = pending->buffer_viewport.buffer.src_width;
      surface->buffer_viewport.buffer.src_height = pending->buffer_viewport.buffer.src_height;

      if (meta_wayland_surface_get_actor (surface))
        {
          meta_surface_actor_set_viewport_src_rect (meta_wayland_surface_get_actor (surface),
                                                    surface->buffer_viewport.buffer.src_x,
                                                    surface->buffer_viewport.buffer.src_y,
                                                    surface->buffer_viewport.buffer.src_width,
                                                    surface->buffer_viewport.buffer.src_height);
        }
    }
                                 ...
                                 enum wl_output_transform transform)
{
  ...
  if (transform >= WL_OUTPUT_TRANSFORM_NORMAL &&
    ...
    {
      if (transform != surface->buffer_viewport.buffer.transform)
        {
    ...
    {
      g_warning ("Trying to set invalid buffer_transform of %d\n", transform);
    }
}
                                               ...
                                               float               *sy)
{
  ...
  if (surface->window &&
  ...
  else
    {
      ...
      *sx /= meta_wayland_surface_get_scale (surface);
      *sy /= meta_wayland_surface_get_scale (surface);
    }
}
                                               ...
                                               float               *y)
{
  ClutterActor *actor =
    CLUTTER_ACTOR (meta_surface_actor_get_texture (meta_wayland_surface_get_actor (surface)));
  ClutterVertex sv = {
    .x = sx * meta_wayland_surface_get_scale (surface),
    .y = sy * meta_wayland_surface_get_scale (surface),
  };
  ClutterVertex v = { 0 };
...
int
meta_wayland_surface_get_scale(MetaWaylandSurface *surface)
{
  return surface->buffer_viewport.buffer.scale;
}

enum wl_output_transform
meta_wayland_surface_get_transform(MetaWaylandSurface *surface)
{
  return surface->buffer_viewport.buffer.transform;
}
...
static int
surface_get_dimension (MetaWaylandSurface *surface, gboolean getWidth)
{
  MetaWaylandBuffer *buffer = surface->buffer_ref.buffer;
  MetaWaylandBufferViewport *vp = &surface->buffer_viewport;
  ...
  if (vp->surface.width > 0)
    {
      return getWidth ? vp->surface.width : vp->surface.height;
    }
  else if (vp->buffer.src_width > 0)
    {
      return getWidth ? vp->buffer.src_width : vp->buffer.src_height;
    }
  else if (buffer)
    {
      int scale;
      ...
      scale = meta_wayland_surface_get_scale (surface);
      ret = getWidth ? meta_wayland_surface_get_buffer_width (surface) : \
                       meta_wayland_surface_get_buffer_height (surface);
      return ret > 0 ? ret / scale : 0;
    }
  else
    {
      return 0;
...
{
  return surface_get_dimension(surface, TRUE);
}

int
meta_wayland_surface_get_height (MetaWaylandSurface *surface)
{
  return surface_get_dimension(surface, FALSE);
}
