...
clutter_offscreen_effect_pre_paint (ClutterEffect *effect)
{
  ...
  /* We store the modelview matrix that was last used when we
   ...
   * This is fine since paint_target runs with the same modelview matrix,
  ...
  /* Set up the viewport so that it has the same size as the stage. Ideally
   * we would use fbo_width and fbo_height, but doing that seems to affect
   * the projection calculation (which is not surprising) and severely
   * distorts/shrinks what ends up on screen. So we keep the stage viewport
   * to ensure the projection is correct. */
