                                             ...
                                             guint              len)
{
  ...
  gint cursor  = clutter_text_get_cursor_position (clutter_text);
  gint start = cursor + offset;
  if (start < 0)
    {
      g_warning ("The offset '%d' of deleting surrounding is larger than the cursor pos '%d'", offset, cursor);
      return;
    }
