                  ...
                  struct libinput_device *libinput_device)
{
  ...
  g_signal_emit_by_name (seat, "device-added", device, master);
}
...
evdev_remove_device (MetaSeatNative        *seat,
                     MetaInputDeviceNative *device_evdev)
{
  ClutterInputDevice *device, *master;

  device = CLUTTER_INPUT_DEVICE (device_evdev);
  master = clutter_input_device_get_associated_device (device);
  seat->devices = g_slist_remove (seat->devices, device);

  g_signal_emit_by_name (seat, "device-removed", device, master);
                               ...
                               ClutterInputDevice *parent)
{
  ...
  for (l = seat_native->devices; l; l = l->next)
    {
      if (clutter_input_device_get_associated_device (l->data) == parent)
...
MetaSeatNative *
meta_seat_native_new (void)
{
  return g_object_new (META_TYPE_SEAT_NATIVE, NULL);
}
