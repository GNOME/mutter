...
struct _MetaShapedTexturePrivate
{
  ...
  gboolean has_viewport_dst;
  int viewport_dst_width, viewport_dst_height;
...
update_size (MetaShapedTexture *stex)
{
  ...
  if (priv->has_viewport_dst)
  ...
  else if (priv->has_viewport_src)
    {
      ...
      dst_width = priv->viewport_src_width / tex_scale;
      dst_height = priv->viewport_src_height / tex_scale;
    }
  else
    {
...
static void
reset_viewport (MetaShapedTexture *stex)
{
  MetaShapedTexturePrivate *priv = stex->priv;

  priv->has_viewport_src = priv->viewport_src_width > 0;
  priv->has_viewport_dst = priv->viewport_dst_width > 0;
}
...
void
meta_shaped_texture_set_viewport_dst (MetaShapedTexture *stex,
                                      int                dst_width,
                                      int                dst_height)
{
  MetaShapedTexturePrivate *priv = stex->priv;

  if (priv->viewport_dst_width != dst_width ||
