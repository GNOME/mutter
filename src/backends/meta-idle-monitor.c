                        ...
                        gpointer user_data)
{
  ...
  if (monitor->inhibited) {
    ...
    g_source_set_ready_time (watch->timeout_source,
                             monitor->last_event_time + watch->timeout_msec * 1000);
  }
}
...
meta_idle_monitor_init (MetaIdleMonitor *monitor)
{
  ...
  /* Monitor inhibitors */
  monitor->session_proxy = g_dbus_proxy_new_for_bus_sync (G_BUS_TYPE_SESSION,
  ...
  g_signal_connect (G_OBJECT (monitor->session_proxy), "g-properties-changed",
                    G_CALLBACK (meta_idle_monitor_inhibited_actions_changed), monitor);

  v = g_dbus_proxy_get_cached_property (monitor->session_proxy, "InhibitedActions");
            ...
            GDestroyNotify             notify)
{
  ...
  if (timeout_msec != 0)
    {
      GSource *source = g_source_new (&idle_monitor_source_funcs, sizeof (GSource));
...
void
_meta_idle_monitor_reset_idletime (MetaIdleMonitor *monitor)
{
