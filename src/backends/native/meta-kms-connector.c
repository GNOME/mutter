                ...
                MetaKmsImplDevice     *impl_device)
{
  ...
  fd = meta_kms_impl_device_get_fd (impl_device);
  edid_blob = drmModeGetPropertyBlob (fd, connector->edid_blob_id);
  if (!edid_blob)
    {
                     ...
                     MetaKmsImplDevice     *impl_device)
{
  ...
  fd = meta_kms_impl_device_get_fd (impl_device);
  tile_blob = drmModeGetPropertyBlob (fd, connector->tile_blob_id);
  if (!tile_blob)
    {
                      ...
                      drmModeRes            *drm_resources)
{
  ...
  common_possible_crtcs = ~(uint32_t) 0;
  common_possible_clones = ~(uint32_t) 0;
                               ...
                               drmModeRes        *drm_resources)
{
  ...
  if (connector->edid_blob_id)
    state_set_edid (state, connector, impl_device);

  if (connector->tile_blob_id)
    state_set_tile_info (state, connector, impl_device);
  ...
  for (i = 0; i < drm_connector->count_props; i++)
    {
      ...
      if ((prop->flags & DRM_MODE_PROP_BLOB) &&
        ...
        connector->tile_blob_id = drm_connector->prop_values[i];
